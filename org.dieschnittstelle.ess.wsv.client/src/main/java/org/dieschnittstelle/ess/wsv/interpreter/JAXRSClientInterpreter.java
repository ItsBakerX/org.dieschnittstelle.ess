    ...
    public Object invoke(Object proxy, Method meth, Object[] args)
        ...
        if (args != null && args.length > 0) {
            if (meth.getParameterAnnotations()[0].length > 0 && meth.getParameterAnnotations()[0][0].annotationType() == PathParam.class) {
                ...
                // get the PathParam annotation to read the param name
                ...
                // replace the {paramName} placeholder in the URL with the actual value
                requestUrl = requestUrl.replace("{" + pathParam.value() + "}", args[0].toString());
                // check if there is a second argument (e.g. updateTouchpoint(id, touchpoint))
                if (args.length > 1) {
        ...
        if(meth.isAnnotationPresent(GET.class)){
            ...
            throw new UnsupportedOperationException("So far we can only handle GET, POST, PUT and DELETE methods");
        }
        ...
        if (requestBodyData != null) {
            ...
            ByteArrayEntity requestBodyDataAsEntity = new ByteArrayEntity(stream.toByteArray());
